# PQforNoise
A Power Query transformation for time series noise data. The transformation aggregates the noise metrics most commonly used used in UK environmental noise assessments (LAeq , LA90 (mode) and LAmax ( 1st and 1th highest). It also produces a time series graph and a pivot chart showing the modal background noise level. 
# 1	Overview 
